...
const SCHEDULE: { t: number; id: string; type: 'beginEvent' | 'repeatEvent' | 'endEvent'; n?: number }[] = [
  { t: 0.00, id: 'a-asm1', type: 'beginEvent' },
  { t: 0.60, id: 'a-asm1', type: 'endEvent' },
  { t: 1.10, id: 'a-asm3', type: 'beginEvent' },
  { t: 2.40, id: 'a-asm4', type: 'endEvent' },
  { t: 2.40, id: 'a-balance', type: 'beginEvent' },
  { t: 2.58, id: 'a-escape', type: 'beginEvent' },
  { t: 3.20, id: 'a-balance', type: 'repeatEvent', n: 1 },
  { t: 7.20, id: 'a-balance', type: 'repeatEvent', n: 6 },
  { t: 7.20, id: 'a-strike', type: 'beginEvent' },
  { t: 8.25, id: 'a-strike', type: 'endEvent' },
  { t: 12.00, id: 'a-balance-hard', type: 'endEvent' },
];

// Timeline ruler lanes (§11): which animation ids each swim lane shows.
const LANES: { name: string; ids: string[] }[] = [
  { name: '装配', ids: ['a-asm1', 'a-asm2', 'a-asm3', 'a-asm4'] },
  { name: '摆轮', ids: ['a-balance', 'a-balance-hard'] },
  { name: '叉瓦', ids: ['a-fork'] },
  { name: '擒纵', ids: ['a-escape'] },
  { name: '分轮', ids: ['a-minute'] },
  { name: '打点', ids: ['a-strike', 'a-hammer'] },
  { name: '凸轮', ids: ['a-cam-a', 'a-cam-b'] },
  { name: '跳秒', ids: ['a-seconds'] },
];
...
const LINKS: { from: string; at: 'begin' | 'end' | 'repeat'; n?: number; offset?: number; to: string; text: string }[] = [
  { from: 'a-asm3', at: 'end', to: 'a-asm4', text: 'a-asm3.end' },
  { from: 'a-asm4', at: 'end', to: 'a-balance', text: 'a-asm4.end' },
  { from: 'a-balance', at: 'begin', to: 'a-fork', text: 'a-balance.begin' },
  { from: 'a-fork', at: 'begin', offset: 0.18, to: 'a-escape', text: 'a-fork.begin+0.18s' },
  { from: 'a-escape', at: 'begin', to: 'a-minute', text: 'a-escape.begin' },
  { from: 'a-balance', at: 'repeat', n: 6, to: 'a-strike', text: 'a-balance.repeat(6)' },
];
...
export async function render(stage: SVGSVGElement): Promise<void> {
  ...
  root.append(fragment(`<defs>
    ...
    <animate id="a-power" href="#power-bar" attributeName="width" attributeType="XML" from="220" to="46" dur="30s" fill="freeze"/>
    <!-- el:discard — removes the shipping clamp from the DOM permanently when assembly step 4 ends. -->
    <discard id="d-clamp" href="#shipping-clamp" begin="a-asm4.end"/>
  </defs>`));
  ...
  root.append(el('text', { id: 'beat-counter', x: 1330, y: 56, 'text-anchor': 'end', 'font-family': FONT_MONO, 'font-size': 18, fill: BRASS_HI }, '拍数 #0'));
  root.append(mono(1330, 72, 'a-balance.onrepeat → repeatEvent 计数', { 'text-anchor': 'end', 'font-size': 10 }));
  ...
  for (const [mode, y] of restartModes) {
    root.append(button(`wind-${mode}`, 70, y, 110, 34, mode, { size: 11 }));
  ...
  barrel.append(spin('a-barrel', 170, 300, 240, 1));
  train.append(barrel);
  ...
  train.append(el('path', { id: 'mainspring', d: spiralPath(170, 300, 20, 76, 3, 40), fill: 'none', stroke: STEEL, 'stroke-width': 2.2, 'stroke-dasharray': '14 5' },
    ...restartModes.map(([mode]) => el('animate', { id: `a-wind-${mode}`, attributeName: 'stroke-dashoffset', from: '0', to: '-57', dur: '0.9s', begin: `wind-${mode}.click`, restart: mode }))));
  train.append(el('circle', { id: 'barrel-cover', cx: 170, cy: 300, r: 30, fill: BRASS, 'fill-opacity': 0.85, stroke: BROWN, 'stroke-width': 1.5 },
    ...
    el('animate', { id: 'a-detach', attributeName: 'cy', values: '300;236;300', keyTimes: '0;0.5;1', calcMode: 'spline', keySplines: SPLINE, dur: '1.6s', begin: 'indefinite' })));
  train.append(mono(120, 404, '#barrel-cover ← a-detach begin="indefinite"', { 'font-size': 9.5 }));
  // Centre wheel (330,350) r=74 · third (450,275) r=52 · fourth (520,380) r=42.
  const centre = el('g', { id: 'centre-wheel', transform: 'rotate(-6 330 350)' }, ...wheelParts(330, 350, 74, 60, { pinion: 16, hub: 9 }), spin('a-centre', 330, 350, 120, -1));
  const third = el('g', { id: 'third-wheel', transform: 'rotate(9 450 275)' }, ...wheelParts(450, 275, 52, 44, { pinion: 14, hub: 7, spokes: 4 }), spin('a-third', 450, 275, 40, 1));
  const fourth = el('g', { id: 'fourth-wheel', transform: 'rotate(-4 520 380)' }, ...wheelParts(520, 380, 42, 36, { pinion: 12, hub: 6, spokes: 4 }), spin('a-fourth', 520, 380, 8, -1));
  ...
  //      translate frame so the accumulating a-escape rotate uses plain angles (per beat +12° net).
  const escapeWheel = el('g', { id: 'escape-wheel', transform: 'rotate(6 580 460)' });
  escapeWheel.append(el('animateTransform', { id: 'a-asm2', attributeName: 'transform', type: 'translate', from: '90 -40', to: '0 0', dur: '0.5s', begin: 'a-asm1.end', additive: 'sum' }));
  ...
  rotor.append(el('animateTransform', {
    id: 'a-escape', attributeName: 'transform', type: 'rotate', values: '0;6;6;12;12', keyTimes: '0;0.075;0.5;0.575;1',
    dur: `${BEAT}s`, begin: 'a-fork.begin+0.18s', repeatCount: 'indefinite', additive: 'sum', accumulate: 'sum',
  }));
  ...
  escapeWheel.append(el('circle', { id: 'escape-jewel', cx: 580, cy: 460, r: 5, fill: RUBY, stroke: BROWN },
    el('animate', { id: 'a-jewel', attributeName: 'r', attributeType: 'XML', values: '5;7.5;5', keyTimes: '0;0.5;1', calcMode: 'spline', keySplines: SPLINE, dur: `${BEAT}s`, begin: 'a-escape.begin', repeatCount: 'indefinite' })));
  root.append(escapeWheel);

  // ---- 4. Pallet fork, pivot (470,520). Locked 0–.225 / .30–.725, impulse flips at .225–.30 and .725–.80 —
  //      exactly when the balance crosses zero (phase .25/.75). begin="a-balance.begin" (av:animate.begin=syncbase).
  const fork = el('g', { id: 'fork', transform: 'translate(470 520)' });
  fork.append(el('animateTransform', { id: 'a-asm3', attributeName: 'transform', type: 'translate', from: '0 60', to: '0 0', dur: '0.5s', begin: 'a-asm2.end', additive: 'sum' }));
  fork.append(el('animateTransform', { id: 'a-fork', attributeName: 'transform', type: 'rotate', values: '-9;-9;9;9;-9;-9', keyTimes: '0;0.225;0.30;0.725;0.80;1', dur: `${BEAT}s`, begin: 'a-balance.begin', repeatCount: 'indefinite', additive: 'sum' }));
  ...
  forkBody.append(el('path', { id: 'fork-lever', class: 'pallet-fork', d: 'M-160,-7 L-150,-3 L-4,-4 L70,-3 L70,3 L-4,4 L-150,3 L-160,7 L-160,3 L-146,0 L-160,-3 Z', stroke: BROWN, 'stroke-width': 1.2 },
    ...
    el('animate', { id: 'a-fork-fill', attributeName: 'fill', attributeType: 'CSS', values: `${STEEL};${BRASS_HI};${STEEL};${BRASS_HI};${STEEL}`, keyTimes: '0;0.225;0.30;0.725;0.80', calcMode: 'discrete', dur: `${BEAT}s`, begin: 'a-balance.begin', repeatCount: 'indefinite' })));
  ...
  root.append(el('circle', { id: 'fork-pivot', cx: 470, cy: 520, r: 7, fill: RUBY, stroke: BROWN, 'stroke-width': 1.2 },
    el('animate', { id: 'a-pivot-drift', attributeName: 'cx', from: '470', to: '476', dur: '12s', fill: 'freeze' }),
    // av:animate.additive=sum — presented cx = base(470→476) + wobble(−1.4…1.4).
    el('animate', { id: 'a-pivot-wobble', attributeName: 'cx', values: '-1.4;1.4;-1.4', dur: `${BEAT}s`, begin: 'a-balance.begin', repeatCount: 'indefinite', additive: 'sum' })));
  root.append(mono(478, 512, 'cx: 470→476 (+wobble ±1.4, additive=sum)', { 'font-size': 9 }));

  // ---- 3. Main balance (300,590) r=115 with a 3.5-turn hairspring (r 18→62). a-balance: keySplines symmetric ease.
  const balance = el('g', { id: 'balance', transform: 'rotate(0 300 590)' });
  balance.append(el('animateTransform', { id: 'a-asm4', attributeName: 'transform', type: 'translate', from: '-70 80', to: '0 0', dur: '0.8s', begin: 'a-asm3.end', additive: 'sum' }));
  balance.append(el('animateTransform', {
    id: 'a-balance', attributeName: 'transform', type: 'rotate', values: '-32 300 590;32 300 590;-32 300 590', dur: `${BEAT}s`,
    calcMode: 'spline', keyTimes: '0;0.5;1', keySplines: SPLINE, repeatCount: 'indefinite', begin: 'a-asm4.end', additive: 'sum',
  }));
  ...
  balance.append(el('path', { id: 'hairspring', d: spiralPath(300, 590, 18, 62, 3.5, 200), fill: 'none', stroke: STEEL, 'stroke-width': 1.4, 'stroke-dasharray': '30 4' },
    ...
    el('animate', { id: 'a-hairspring', attributeName: 'stroke-dashoffset', values: '0;-16;0', keyTimes: '0;0.5;1', calcMode: 'spline', keySplines: SPLINE, dur: `${BEAT}s`, begin: 'a-balance.begin', repeatCount: 'indefinite' })));
  ...
  root.append(balance);
  // Balance cock (drops in first: a-asm1 begin="0s") and the staff jewel above everything.
  const cock = el('g', { id: 'balance-cock' });
  cock.append(el('animateTransform', { id: 'a-asm1', attributeName: 'transform', type: 'translate', from: '0 -70', to: '0 0', dur: '0.6s', begin: '0s' }));
  ...
  root.append(cock);
  // Orange shipping clamp: <discard> in defs removes it at a-asm4.end; the <set> is the Firefox fallback.
  root.append(el('g', { id: 'shipping-clamp', transform: 'rotate(20 192 551)' },
    ...
    el('set', { id: 's-clamp', attributeName: 'opacity', to: '0', begin: 'a-asm4.end', fill: 'freeze' })));
  root.append(label(416, 556, '↑ visibility=hidden 夹板 #top-plate：隐藏但仍占位（getBBox 490×320），红宝石轴承 visibility=visible 仍显影', { 'font-size': 10, fill: SAGE }));
  ...
  hard.append(el('animateTransform', { id: 'a-balance-hard', attributeName: 'transform', type: 'rotate', values: '-32 560 650;32 560 650;-32 560 650', dur: '2.4s', calcMode: 'linear', keyTimes: '0;0.5;1', repeatDur: '9.6s', begin: 'a-asm4.end', additive: 'sum' }));
  ...
  root.append(hard);
  root.append(label(560, 716, '硬游丝 / 线性 / 3× 慢', { 'text-anchor': 'middle', fill: BRASS_HI }));
  root.append(mono(560, 728, 'dur=2.4s calcMode=linear repeatDur=9.6s', { 'text-anchor': 'middle', 'font-size': 9.5 }));
  root.append(label(300, 720, '主摆轮 · 3.5 圈游丝', { 'text-anchor': 'middle', fill: BRASS_HI }));
  root.append(mono(300, 732, 'dur=0.8s calcMode=spline keySplines=.42 0 .58 1', { 'text-anchor': 'middle', 'font-size': 9.5 }));
  ...
  const dummyGroup = el('g', { id: 'dummy-group' },
    ...
    el('circle', { cx: 528, cy: 168, r: 3.5, fill: STEEL, stroke: BROWN }),
    el('circle', { cx: 700, cy: 168, r: 3.5, fill: STEEL, stroke: BROWN }));
  root.append(dummyGroup);
  root.append(label(520, 150, 'display=none 夹板 #dummy-plate → 父组 getBBox 收缩为两枚螺钉，探针也不命中', { 'font-size': 10, fill: SAGE }));
  ...
  cmpA.append(el('animateTransform', { id: 'a-cmp-spin', attributeName: 'transform', type: 'rotate', values: '0 560 200;360 560 200', dur: '6s', repeatCount: 'indefinite', additive: 'sum' }));
  // No additive → replace: this translate throws away both the base rotate(-8) and the spin above it.
  cmpA.append(el('animateTransform', { id: 'a-cmp-bob', attributeName: 'transform', type: 'translate', values: '0 0;0 6;0 0', dur: `${BEAT}s`, repeatCount: 'indefinite' }));
  ...
  cmp.append(cmpA);
  const cmpB = el('g', { id: 'cmp-animate-transform', transform: 'rotate(0 650 200)' }, ...wheelParts(650, 200, 28, 22, { spokes: 4, hub: 5 }));
  // concept:animate-transform-requires-animatetransform — <animate attributeName="transform"> is ignored.
  cmpB.append(el('animate', { id: 'a-cmp-noop', attributeName: 'transform', values: 'rotate(0 650 200);rotate(360 650 200)', dur: '4s', repeatCount: 'indefinite' }));
  cmp.append(cmpB);
  cmp.append(label(560, 254, '无 additive → replace', { 'text-anchor': 'middle', 'font-size': 10 }));
  cmp.append(label(560, 266, '丢失底旋转，只剩顶替的上下顶动', { 'text-anchor': 'middle', 'font-size': 10, fill: SAGE }));
  cmp.append(mono(650, 244, 'animate transform', { 'text-anchor': 'middle', 'font-size': 9.5 }));
  cmp.append(label(650, 256, '完全不动', { 'text-anchor': 'middle', 'font-size': 10, fill: SAGE }));
  root.append(cmp);
  ...
  dial.append(el('g', { id: 'hour-hand', transform: 'rotate(0 830 300)' },
    ...
    el('animateTransform', { id: 'a-hour', attributeName: 'transform', type: 'rotate', from: '0 830 300', to: '360 830 300', dur: '120s', repeatCount: 'indefinite', additive: 'sum' })));
  ...
  minuteHand.append(el('animateTransform', { id: 'a-minute-sweep', attributeName: 'transform', type: 'rotate', from: '0 830 300', to: '360 830 300', dur: '60s', repeatCount: 'indefinite', additive: 'sum' }));
  minuteHand.append(el('animateTransform', { id: 'a-minute-orbit', attributeName: 'transform', type: 'translate', values: '0 0;0 1.5;0 0', dur: `${BEAT}s`, repeatCount: 'indefinite', additive: 'sum' }));
  ...
  minuteFrame.append(el('animateTransform', { id: 'a-minute', attributeName: 'transform', type: 'rotate', values: '0;1.5;1.5', keyTimes: '0;0.075;1', dur: `${BEAT}s`, begin: 'a-escape.begin', repeatCount: 'indefinite', additive: 'sum', accumulate: 'sum' }));
  ...
  dial.append(el('circle', { id: 'seconds-glow', cx: 830, cy: 400, r: 52, fill: STEEL, opacity: 0.25 },
    el('animate', { id: 'a-glow', attributeName: 'opacity', attributeType: 'CSS', values: '0.25;0.6;0.25', dur: '1.6s', repeatCount: 'indefinite' })));
  ...
  dial.append(el('g', { id: 'seconds-hand', transform: 'rotate(0 830 400)' },
    ...
    el('animateTransform', { id: 'a-seconds', attributeName: 'transform', type: 'rotate', values: secondsValues, calcMode: 'discrete', dur: '8s', begin: '0s', repeatCount: 'indefinite', additive: 'sum' })));
  ...
  // Power-reserve bar (700,468) 220 wide — animated remotely from <defs> (see a-power).
  ...
  root.append(label(652, 528, `装饰星轮走 CSS @keyframes；机构走 SMIL 时基。减动（prefers-reduced-motion）：${reduce ? '开 — 装饰停转，时基冻结于基准帧' : '关'}`, { 'font-size': 10, fill: SAGE }));
  root.append(label(652, 542, '导出帧：真实运行到 T0=7.40s 让 begin/repeat/end 事件真实发生，再 pauseAnimations()+setCurrentTime(7.40) 定帧', { 'font-size': 10, fill: SAGE }));
  ...
  // ---- 6. Striking work. a-strike releases on the 6th balance repeat or on the silence lever click.
  const strike = el('g', { id: 'strike' });
  strike.append(label(1046, 104, '打点机构 · 第六拍自动释放 / 止打杆一按即停', { fill: BRASS_HI }));
  strike.append(mono(1046, 117, 'begin="a-balance.repeat(6);silence-lever.click" repeatCount="3" end="silence-lever.click"', { 'font-size': 9 }));
  ...
  countFrame.append(el('animateTransform', {
    id: 'a-strike', attributeName: 'transform', type: 'rotate', from: '0', to: '40', dur: '0.35s', repeatCount: '3',
    begin: 'a-balance.repeat(6);silence-lever.click', end: 'silence-lever.click', additive: 'sum', accumulate: 'sum', onbegin: '__chronoStrikeBegin(event)',
  }));
  ...
  strike.append(el('rect', { id: 'strike-lock', x: 1166, y: 150, width: 36, height: 14, rx: 3, fill: BRASS, stroke: BROWN },
    // el:set / at:set.to — snaps to ruby at a-strike.begin and stays (no interpolation).
    el('set', { id: 's-lock', attributeName: 'fill', to: RUBY, begin: 'a-strike.begin', fill: 'freeze' })));
  ...
  hammer.append(el('animateMotion', { id: 'a-hammer-rest', path: HAMMER_PATH, keyPoints: '0;0', keyTimes: '0;1', calcMode: 'linear', rotate: 'auto', dur: '0.01s', begin: '0s', fill: 'freeze' }));
  // at:animateMotion.path + av:animateMotion.rotate=auto — the strike hammer follows the inline Bézier, head first.
  hammer.append(el('animateMotion', { id: 'a-hammer', path: HAMMER_PATH, rotate: 'auto', dur: '0.35s', repeatCount: '3', begin: 'a-strike.begin', end: 'silence-lever.click' }));
  ...
  strike.append(el('circle', { id: 'gong-ripple', cx: 1244, cy: 240, r: 17, fill: 'none', stroke: BRASS_HI, 'stroke-width': 1.5, opacity: 0.7 },
    el('animate', { id: 'a-gong', attributeName: 'r', values: '17;32', dur: '0.35s', repeatCount: '3', begin: 'a-strike.begin', end: 'silence-lever.click' }),
    el('animate', { attributeName: 'opacity', values: '0.9;0', dur: '0.35s', repeatCount: '3', begin: 'a-strike.begin', end: 'silence-lever.click' })));
  ...
  strike.append(button('silence-lever', 1046, 268, 120, 28, '止打杆 SILENCE', { font: FONT_CJK, size: 11, fill: '#4b3a2a' }));
  strike.append(el('text', { id: 'mute-plaque', x: 1178, y: 288, 'font-family': FONT_CJK, 'font-size': 12, fill: RUBY, visibility: 'hidden' }, '● 静音',
    ...
    el('set', { id: 's-mute', attributeName: 'visibility', to: 'visible', begin: 'silence-lever.click', fill: 'freeze' })));
  root.append(strike);
  ...
  followerA.append(el('animateMotion', { id: 'a-cam-a', dur: '9s', repeatCount: 'indefinite', calcMode: 'linear', rotate: 'auto', keyPoints: '0;0.25;0.25;1', keyTimes: '0;0.3;0.55;1', begin: '0s' }, el('mpath', { href: '#eot-cam-profile' })));
  ...
  followerB.append(el('animateMotion', { id: 'a-cam-b', dur: '4s', repeatCount: '2.5', calcMode: 'linear', rotate: 'auto-reverse', keyPoints: '1;0', keyTimes: '0;1', begin: '0s', fill: 'freeze' }, el('mpath', { href: '#eot-cam-profile' })));
  ...
  cam.append(label(1046, 536, '均时差凸轮 EoT cam — keyPoints 管距离（弧长 s），keyTimes 管时间；两枚从动件共用 <mpath href="#eot-cam-profile">', { 'font-size': 10, fill: SAGE }));
  ...
  root.append(label(1030, 556, 'SMIL 2026：Chrome / Firefox / Safari 均原生渲染，无 polyfill；', { 'font-size': 10.5, fill: CREAM }));
  root.append(label(1030, 570, `discard：${discardSupported ? '支持（元素将被真正移出 DOM）' : '本引擎忽略，已用 set 兜底'}`, { 'font-size': 10.5, fill: discardSupported ? BRASS_HI : RUBY }));
  ...
  log.append(el('text', { id: 'log-title', x: 1040, y: 596, 'font-family': FONT_CJK, 'font-size': 12, fill: BRASS_HI }, '事件日志 · beginEvent / repeatEvent / endEvent'));
  ...
  station.append(button('btn-begin', 1046, 786, 88, 22, 'beginElement()', { size: 9.5 }));
  station.append(button('btn-begin-at', 1142, 786, 92, 22, 'beginElementAt(.5)', { size: 9.5 }));
  station.append(button('btn-end', 1242, 786, 88, 22, 'endElement()', { size: 9.5 }));
  station.append(mono(1046, 780, '#a-detach（barrel-cover 起盖）：释放 / 预约 0.5s / 急停', { 'font-size': 9.5, 'font-family': FONT_CJK }));
  ...
  station.append(button('freeze-lever', 1046, 850, 76, 18, '冻结 / 运行', { font: FONT_CJK, size: 10, fill: '#4b3a2a' }));
  ...
  station.append(label(1046, 812 - 4, '', { id: 'slider-hint', 'font-size': 9, fill: SAGE }));
  root.append(station);

  // ---- 16. Pointer HUD (top-most, pointer-events:none).
  const hud = el('g', { id: 'hud', transform: 'translate(822 496)' });
  ...
  const refreshTiming = (): boolean => {
    ...
    for (const a of animEls) {
      ...
      } catch { /* unresolved interval (e.g. a-strike before the 6th repeat) — keep the last known value */ }
    }
    return changed;
  };
  const drawBars = (): void => {
  ...
  // property/attribute forms (a-balance.onrepeat property, a-strike onbegin= XML attribute).
  ...
  let beats = 0;
  const balanceAnim = byId.get('a-balance')!;
  ...
  window.__chronoStrikeBegin = () => { fire('a-strike', 'beginEvent'); $('log-title').textContent = '事件日志 · 打点中（onbegin= XML 属性命中）'; };
  byId.get('a-strike')!.addEventListener('endEvent', () => { $('log-title').textContent = '打点结束 · a-strike endEvent'; });
  // restart triple counters via beginEvent.
  for (const [mode] of restartModes) byId.get(`a-wind-${mode}`)!.addEventListener('beginEvent', () => { const c = $(`cnt-${mode}`); c.textContent = `×${Number(c.textContent!.slice(1)) + 1}`; });
  ...
  $('freeze-lever').addEventListener('click', () => { if (stage.animationsPaused()) stage.unpauseAnimations(); else stage.pauseAnimations(); syncUi(); });
  const detach = byId.get('a-detach')!;
  $('btn-begin').addEventListener('click', () => detach.beginElement());
  $('btn-begin-at').addEventListener('click', () => detach.beginElementAt(0.5));
  $('btn-end').addEventListener('click', () => detach.endElement());
  for (const id of ['freeze-lever', 'btn-begin', 'btn-begin-at', 'btn-end', 'silence-lever', 'wind-always', 'wind-whenNotActive', 'wind-never']) {
  ...
  const describe = (): void => {
    ...
    const anim = animEls.find(a => a.targetElement && hit && (a.targetElement === hit || hit.contains(a.targetElement) || a.targetElement.contains(hit)) && a.id);
    ...
    const lines = [
      ...
      `命中: #${hit?.id ?? '—'}`,
      anim ? `动画: #${anim.id} → targetElement #${anim.targetElement?.id || anim.targetElement?.localName}` : '动画: —（无 SMIL 驱动）',
      `getStartTime ${startText} · getSimpleDuration ${durText}`,
  ...
  const syncUi = (): void => {
    ...
    $('slider-hint').textContent = `拖动滑轨 ↔ setCurrentTime(t)，t∈[0,${T_MAX}]；把手 x = 1046 + t/12·284`;
